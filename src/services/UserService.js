...
export const getAllUser = async (page, limit, type, search) => {
    let res = {};
    if (search?.length > 0) {
        res = await axios.get(
            `${process.env.REACT_APP_API_URL}/user/getall?filter=${type}&filter=${search}&page=${page}&limit=${limit}`
        );
    } else {
        res = await axios.get(
            `${process.env.REACT_APP_API_URL}/user/getall?page=${page}&limit=${limit}`
        );
    }
    return res.data;
};
